...
export async function handleTranscriptRoutes(
    ...
    // ----------------------------
    // 📄 List Transcripts (FIXED)
    // ----------------------------
    if (request.method === "GET" && url.pathname === "/transcripts") {
        try {
            const { results } = await env.DB.prepare(
                `
        SELECT id, r2_key, created_at, summary
        FROM transcripts
        ...
        `
            ...
            return new Response(
                JSON.stringify({ error: "Failed to list transcripts" }),
                {
                    status: 500,
                    headers: { "Content-Type": "application/json" },
                }
    ...
    if (request.method === "GET" && url.pathname.startsWith("/transcript/")) {
        ...
        try {
            ...
            return new Response(
                JSON.stringify({ error: "Failed to retrieve transcript" }),
                {
                    status: 500,
                    headers: { "Content-Type": "application/json" },
                }
